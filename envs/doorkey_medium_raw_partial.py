...
class DoorKeyCustomEnv(MiniGridEnv):
    ...
    def __init__(self, size=8,input_maxstep=640):
        super().__init__(
            grid_size=size,
            max_steps=input_maxstep, #10*size*size,
            agent_view_size= 9,
...
class DoorKeyMediumRawPartial(gym.Env):
    def __init__(self,input_maxstep):
        self.input_maxstep = input_maxstep
        self.env = DoorKeyCustomEnv(input_maxstep=input_maxstep) #gym.make('MiniGrid-DoorKey-8x8-v0')  
        self.env.width= 10
        self.env.height = 10 
        self.tile_size = 1
